...
def load(nb_file=1):
    df_org, cont_feat, bin_feat = get_NASA(nb_file)
    df_org = df_org[~df_org.isin([np.nan, np.inf, -np.inf]).any(axis=1)]
    time_scale = 100
    ...
    for i in np.arange(n_samples):
        ...
        # merge with sampling time grid defined above
        df_idx = pd.merge(df_times, df_[df_.id == idx], how="left", on=["id", "times"]).ffill().bfill()
