...
CDF와 categorical PMF는 연결된 표현이므로, 하나를 본문 기본 구현으로 두고 다른 표현은 변형으로 비교. 이거는 어떤 걸 의미하는거야? 이산 grid로 hazard 표현 vs 연속 grid 로 hazrd 표현을 말하는거지?

네가 말한 “이산 시간 격자의 hazard 모델 vs 연속시간 hazard 모델”로 구분하는 게 더 정확해. 내가 앞에서 CDF와 categorical PMF를 서로 다른 구현처럼 표현한 건 부정확했어.

구분해야 하는 축이 두 개야.

1. 시간을 어떻게 표현하는가?

구분	이산 시간 격자 모델	연속시간 모델
시간 표현	0–1시간, 1–2시간 등의 bin	임의의 실수 시간 \(\tau\)
hazard의 의미	해당 bin에서 사건이 발생할 조건부 확률	해당 순간의 사건 발생률
출력 예시	bin별 hazard \(h_k(s)\)	시간의 함수 \(\lambda(\tau\mid s)\)
실제 \(D\)만큼 이동	격자에 맞는 투영·보간 등이 필요	\(\tau-D\)에서 분포를 직접 평가

여기서 “연속 grid”보다는 연속시간 함수라고 부르면 좋아. 연속시간 모델도 계산할 때는 유한한 평가 시점을 사용할 수 있어.

2. 그 분포를 어떤 함수로 표현하는가?

CDF는 이산·연속 모델 모두에서 정의할 수 있어.

이산 모델에서는 hazard를 PMF와 CDF로 바꿀 수 있어.

$$ p_k(s)=h_k(s)\prod_{j<k}(1-h_j(s)), \qquad F(t_k\mid s)=\sum_{j\le k}p_j(s). $$

연속시간 hazard 모델에서는 다음처럼 CDF를 구해.

$$ F(\tau\mid s) = 1-\exp\left[-\int_0^\tau\lambda(u\mid s)\,du\right]. $$

따라서 “PMF를 쓰는 모델 vs CDF를 쓰는 모델”이 반드시 별개의 모델인 건 아니야. 같은 이산 모델이 PMF를 출력하고, 이를 누적한 CDF로 TD loss를 계산할 수도 있어.

우리 연구에서는 이렇게 정리하면 돼.

이산 격자 구현: bin별 hazard 또는 PMF로 분포를 표현하고, 실제 \(D\)만큼 이동한 target을 격자에 투영.
연속시간 구현: 임의의 시간에서 평가 가능한 유효한 CDF를 만들고, \(\tau-D\)에서 target을 평가. 이 CDF를 만드는 한 가지 방법이 연속시간 hazard 모델이야.

현재 노트북은 첫 번째인 ‘categorical PMF + CDF 기반 TD’ 구현이야. 연속시간 hazard를 학습하는 구현은 아니야.

논문에서는 이 두 구현 중 하나를 기본으로 선택하고 다른 것을 비교하면 돼. 두 구현 모두 실제 \(D\)를 반영할 수 있으므로, 이산 격자를 사용한다고 semi-Markov 설정이 아닌 것은 아니야.
